...
function populateTasks() {
  ...
  tasks.push(
    createTask("TASK 2", "Implement task form", "Vishnu", "25/4/2023", "to-do")
  );
  tasks.push(
    createTask(
      "TASK 3",
      "Implement task layout",
      "Parisa",
      "23/4/2023",
      "in-progress"
    )
  ...
  tasks.push(
    createTask("TASK 4", "Style the page", "Vishnu", "17/4/2023", "done")
  );
  tasks.push(
    createTask("TASK 5", "Requirement gathering", "Parisa", "20/4/2023", "done")
  );
  tasks.push(
    createTask(
      "TASK 6",
...
function displayTasks(filterBy) {
  ...
  if (filterBy) {
    ...
    if (filteredTasks.length > 0) {
      taskContainer.innerHTML = getTasks()
        .filter((task) => task.status === filterBy)
        .map(generateTaskHtml)
        .join("");
    } else {
  ...
  document
    .querySelectorAll(".dropdown-menu.filter-status li a")
    .forEach((item) => {
...
function generateTaskHtml(task) {
  return (
    `<div class="col-md-6 col-lg-4 col-12" id="${task.id}-original">  
                        <div class="card">
                        <div class="change">
                        <span class="material-symbols-outlined avatar" role="button" title="${
                          task.assignedTo
                        }">person</span>
                        </div>
                        <div class="card-body">
                                ...
                                <br/>
                                <span title="delete" role="button" class="material-symbols-outlined delete-btn" id="${
                                  task.id
                                }">delete</span>
                                <span title="edit" role="button" class="material-symbols-outlined edit-btn" id="${
                                  task.id
                                }" >stylus</span>
                                <span role="button" style="display:${
                                  task.status === "done"
                                    ? "none"
                                    : "inilne-block"
                                }" class="material-symbols-outlined mark-as-done" title="Mark as done" id="${
      task.id
    }">done</span>
                                </div>
                            </div>
...
const addNewTask = () => {
  ...
  if (userDescription.value.length <= 15) {
    validInputs = false;
    invalidDescription.innerHTML =
      "Please provide valid description with at least 15 chars";
  }

  if (userAssignTo.value.length <= 8) {
    validInputs = false;
    invalidAssignedTo.innerHTML =
      "Please provide valid assigned to value with at least 8 chars";
  }
...
function activeSaveButton(event) {
  ...
  const saveDescription = document.getElementById(
    `${taskId}-description`
  ).value;
